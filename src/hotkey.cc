...
BOOL CALLBACK SearchBrowserWindow(HWND hwnd, LPARAM lparam) {
  if (IsWindowVisible(hwnd) && IsBrowserWindow(hwnd)) {
    DWORD pid;
...
void HideAndShow() {
  auto browser_pids = GetAppPids();
  if (!is_hide) {
    original_mute_states.clear();
    EnumWindows(SearchBrowserWindow, 0);
    MuteProcess(browser_pids, true, true);
  } else {
    ...
    hwnd_list.clear();
    MuteProcess(browser_pids, false);
    original_mute_states.clear();
  }
  is_hide = !is_hide;
}
