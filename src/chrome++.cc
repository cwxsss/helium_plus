...
void ChromePlus() {
  ...
  // Pre-initialize UI Automation before installing hooks, so the first
  // mouse click does not trigger synchronous UIA initialization inside the
  // hook callback, which would delay WM_LBUTTONUP and cause Chrome's
  // views::Button to cancel the click action.
  PreInitializeUia();
